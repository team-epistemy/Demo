...
// Case-based question sets: a short context, then ordered parts that build on it.
// Scenarios are novel (not the slide examples) but test the same Operations concepts.
const OPS_CASES = [
  {
    id: "clinic",
    title: "Case · Vaccination Clinic",
    topic: "Rate & Capacity",
    context: "A pop-up vaccination clinic runs three stations in sequence, one worker each. Check-in takes 2 minutes per patient. The injection takes 3 minutes. After the shot, patients must sit in observation for 15 minutes, and there are 5 observation chairs.",
    parts: [
      "What is the capacity, in patients per hour, of each of the three stations? Show how you get each one.",
      "Which station is the bottleneck, and what is the clinic's process capacity?",
      "If 24 patients per hour arrive, what is the implied utilization of each station, and where do queues form?",
      "Propose one lever to raise the clinic's capacity, and state its trade-off.",
    ],
  },
  {
    id: "returns",
    title: "Case · Returns Center",
    topic: "Little's Law",
    context: "An online retailer's returns center takes in 5,000 returned items per day. On a normal week, about 20,000 items are somewhere in the pipeline (received but not yet processed) at any moment.",
    parts: [
      "Using Little's Law, what is the average time an item spends in the returns pipeline?",
      "Express that pipeline as days-of-supply and as annual inventory turns, and reconcile the two.",
      "A redesign cuts average flow time to 2.5 days at the same arrival rate. What is the new average pipeline inventory, and why does that free up cash and space?",
      "In the post-holiday peak, returns surge to 9,000 per day, but the building only holds 20,000 items. Plugging those into Little's Law gives about 2.2 days. Is that a valid flow time here? Explain.",
    ],
  },
];
...
// Parse a plain-language rubric (one criterion per line, optional points) into
// structured criteria. Accepts "Name - 3", "Name | 3", "Name (3)", "Name: 3 pts", or bare "Name".
function parseRubric(text) {
  if (!text || !text.trim()) return [];
  return text.split(/\n+/).map(l => l.trim()).filter(Boolean).map((line, i) => {
    let name = line, points = null;
    const m = line.match(/^(.*?)\s*[|\-–:(]\s*(\d+(?:\.\d+)?)\s*(?:pts?|points?)?\s*\)?\s*$/i);
    if (m) { name = m[1].trim(); points = parseFloat(m[2]); }
    name = name.replace(/^[-•*\d.)\s]+/, "").trim();
    return { id: "c" + i, name: name || `Criterion ${i + 1}`, points: points != null ? points : 5 };
  }).filter(c => c.name);
}
...
function StepConfigExam({ topics, onNext }) {
  ...
  const parsedRubric = parseRubric(rubricText);
  const rubricTotal = parsedRubric.reduce((s, c) => s + c.points, 0);
  ...
  async function handleGenerate() {
    setGenerating(true);
    const cfg = { qCount, examLen, difficulty, selectedTopics, topics, rubric: rubricText, includeCase };
  ...
  return (
    <div className="card">
      ...
      <label style={{ marginTop: 22 }}>Evaluation Rubric <span style={{ color: T.muted, fontWeight: 400 }}>· optional</span></label>
      <p style={{ fontSize: 12, color: T.inkLight, margin: "0 0 8px", lineHeight: 1.55 }}>
        Paste your grading criteria, one per line. Add points with a dash or bar (for example "Correct formula - 3").
        Each answer is then scored against these criteria alongside the Epistemic Depth Score.
      </p>
      <textarea
        value={rubricText}
        onChange={e => setRubricText(e.target.value)}
        placeholder={"Correctly identifies the concept - 3\nExplains the reasoning, not just states it - 4\nUses a valid method or example - 3"}
        style={{ width: "100%", minHeight: 90, padding: "11px 13px", border: `1px solid ${T.border}`, borderRadius: 8,
          fontFamily: "Inter, sans-serif", fontSize: 14, color: T.ink, background: T.parchment, resize: "vertical", outline: "none" }}
      />
      {parsedRubric.length > 0 && (
        <div style={{ marginTop: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: T.muted, marginBottom: 6 }}>
            Parsed criteria · {rubricTotal} pts total
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {parsedRubric.map(c => (
              <span key={c.id} style={{ fontSize: 12, background: T.white, border: `1px solid ${T.border}`,
                borderRadius: 20, padding: "4px 11px", color: T.ink }}>
                {c.name} <strong style={{ color: T.gold }}>{c.points}</strong>
              </span>
            ))}
          </div>
        </div>
      ...
      <label style={{ marginTop: 22 }}>Case-Based Questions</label>
      <div
        onClick={() => setIncludeCase(v => !v)}
        style={{ display: "flex", alignItems: "flex-start", gap: 12, cursor: "pointer",
          background: T.white, border: `1px solid ${includeCase ? T.gold : T.border}`,
          borderRadius: 10, padding: "12px 14px", marginTop: 2 }}>
        <div style={{ flexShrink: 0, marginTop: 1, width: 18, height: 18, borderRadius: 5,
          border: `1.5px solid ${includeCase ? T.gold : T.border}`, background: includeCase ? T.gold : "transparent",
          color: "white", fontSize: 12, fontWeight: 800, lineHeight: "15px", textAlign: "center" }}>
          {includeCase ? "✓" : ""}
        </div>
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: T.navy }}>Add a case-based question set</div>
          <div style={{ fontSize: 12, color: T.inkLight, marginTop: 2, lineHeight: 1.5 }}>
            Appends a short case with a shared context and several parts that build on it, testing applied reasoning across concepts.
          </div>
        </div>
      </div>
...
const DISCIPLINES = [
  ...
  },
  {
    ...
    subtitle: "Leadership & Ethics",
    instructor: "Prof. Harris Sondak",
    icon: "⚖️",
    color: "#173A2C",
    accent: "#3E9E74",
    scenario: "You are a Socratic examiner for an undergraduate Foundations of Business course at Berkeley Haas, focused on leadership and ethics. Probe understanding of the Berkeley Haas Defining Leadership Principles, ethical frameworks (utilitarian and deontological), gray-area corporate dilemmas, and designing ethical organizations. These questions rarely have one right answer, so press the student to reason and defend their judgment. One question per turn, 2–3 sentences.",
    nodes: [
      { id: "principles", label: "Defining Principles", x: 150, y: 70 },
      { id: "frameworks", label: "Ethical Frameworks", x: 560, y: 70 },
      { id: "qsq", label: "Question Status Quo", x: 140, y: 210 },
      { id: "by", label: "Beyond Yourself", x: 330, y: 210 },
      { id: "util", label: "Utilitarian", x: 450, y: 210 },
      { id: "deont", label: "Deontological", x: 620, y: 210 },
      { id: "dilemma", label: "Gray-Area Cases", x: 330, y: 340 },
      { id: "orgdesign", label: "Ethical Org Design", x: 560, y: 340 },
    ],
    edges: [["principles","qsq"],["principles","by"],["frameworks","util"],["frameworks","deont"],["by","dilemma"],["util","dilemma"],["deont","dilemma"],["principles","dilemma"],["dilemma","orgdesign"]],
  },
];
...
const ExamStore = { trackId: null, trackLabel: null, questions: null, rubric: null, caseSet: null };
...
const EXAM_CONTEXT = {
  ...
  ugba: "Welcome to your Foundations of Business oral examination, focused on leadership and ethics. We'll work through the Berkeley Haas Defining Leadership Principles and how to reason about gray-area corporate dilemmas using ethical frameworks. There is rarely one right answer here, so reason out loud and defend your judgment. Let's begin.",
};
...
const STUDENT_BANKS = {
  ...
  // ── Foundations of Business · Leadership & Ethics (default) ──
  ugba: [
    { topic: "Defining Principles", q: "Name the four Berkeley Haas Defining Leadership Principles, and explain what 'Question the Status Quo' actually asks of a leader in practice." },
    { topic: "Defining Principles", q: "'Confidence Without Attitude' and 'Beyond Yourself' can pull in different directions. Give a situation where they tension, and how you'd resolve it." },
    { topic: "Ethical Frameworks", q: "Distinguish a utilitarian (teleological) from a duty-based (deontological) approach to a management decision, and give a case where they reach opposite conclusions." },
    { topic: "Ethical Frameworks", q: "A manager can hit a quarterly target by delaying a safety fix that will probably harm a few customers. Analyze the decision through both a utilitarian and a deontological lens." },
    { topic: "Gray-Area Dilemmas", q: "Walk me through how you'd structure the analysis of a gray-area corporate dilemma where no option is clearly right." },
    { topic: "Ethical Org Design", q: "The argument is that good employees still make bad choices. Explain why, and name one structural change that reduces unethical behavior." },
    { topic: "Ethical Org Design", q: "Why is hiring 'ethical people' insufficient on its own, and what does deliberately designing an ethical organization add?" },
    { topic: "Stakeholders", q: "Contrast a shareholder-primacy view with a stakeholder view when a plant closure would cut costs but devastate a town." },
    { topic: "Question the Status Quo", q: "Give an example of a business norm a leader should question, and make the ethical case for changing it." },
    { topic: "Beyond Yourself", q: "'Beyond Yourself' asks leaders to act for the broader good. How would you operationalize that in a real resource-allocation decision?" },
  ],
};
        ...
        {[...DISCIPLINES].sort((a, b) => (a.id === "ops" ? -1 : b.id === "ops" ? 1 : 0)).map(d => {
          ...
          return (
            <div key={d.id}
              ...
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginBottom: d.instructor ? 4 : 20 }}>{subtitle}</div>
              {d.instructor && (
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginBottom: 20 }}>{d.instructor}</div>
              )}
...
function OralExam({ discipline, studentName, onBack, previewMode }) {
  ...
  const rubricMax = rubric.reduce((s, c) => s + (c.points || 0), 0);
  const MAX_Q_TURNS = 3;

  const openingFor = (i) => {
    const b = bank[i];
    if (b.context) return `Part ${b.part}. ${b.q}`;
    return i === 0
      ? `${examContext}\n\nQuestion 1. ${b.q}`
      : `Question ${i + 1}. ${b.q}`;
  };
  ...
  const [qData, setQData]       = useState(() => bank.map((qq, i) => ({
    ...
    attempts: 0, attempted: false, done: false, score: 0, visited: i === 0, history: [], rubric: [],
  })));
  ...
  async function handleAnswer(){
    ...
    const system =
      ...
      (rubric.length
        ? `A grading rubric is provided as JSON: ${JSON.stringify(rubric.map(c => ({ criterion: c.name, max: c.points })))}. ` +
          `If (and only if) the student actually answered, ALSO grade the answer against each rubric criterion: award an integer score from 0 to that criterion's max with a one-line justification. ` +
          `Rubric grading is separate from the depth assessment. If the student did not answer, return an empty rubric array. `
        : ``) +
      ...
      (rubric.length ? `, "rubric": [{"criterion": "<name>", "score": <integer>, "max": <max>, "note": "<one short line>"}]` : ``) +
      `}`;

    let ctx = "";
    if (bank[current].context) ctx += `Case context (applies to this part): ${bank[current].context}\n\n`;
    ctx += `Exam question: ${askedQ}\n\n`;
    ...
    let clarify=false, rephrase="", answered=true, adequate=true, feedback="", probe="", modelOk=false, rubricResult=[];
    try{
      const data = await callModel({ model:"claude-sonnet-4-6", max_tokens:700, system, messages:[{role:"user",content:ctx}] });
      ...
      if(Array.isArray(parsed.rubric)) rubricResult=parsed.rubric.map(r=>({
    ...
    setQData(prev=>prev.map((q,idx)=>{
      ...
      return {
        ...
        history: newHist,
        rubric: rubricResult.length ? rubricResult : q.rubric,
      };
    }));
    setLoading(false);
  ...
  return (
    <div style={{ width:"100%", maxWidth:1150, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", gap:20, alignItems:"flex-start", background:T.white, border:`1px solid ${T.border}`,
        ...
        <div style={{ flex:1, minWidth:0 }}>
          {bank[current].context && (
            <div style={{ background:"#FBF6EA", border:`1px solid ${T.gold}`, borderRadius:10,
              padding:"12px 14px", marginBottom:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:6 }}>
                <span style={{ fontSize:11, fontWeight:800, textTransform:"uppercase", letterSpacing:"0.05em", color:T.gold }}>
                  📋 {bank[current].caseTitle}
                </span>
                <span style={{ fontSize:11, color:T.muted }}>Part {bank[current].part} of {bank[current].partsTotal}</span>
              </div>
              <div style={{ fontSize:13, color:T.ink, lineHeight:1.6 }}>{bank[current].context}</div>
            </div>
          )}
        ...
        <div style={{ width:290, flexShrink:0 }}>
          <div style={{ border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
            ...
            {/* Rubric score for the current question (teacher-supplied rubric) */}
            {rubric.length > 0 && (() => {
              const scored = cur.rubric && cur.rubric.length;
              const rows = scored
                ? cur.rubric
                : rubric.map(c => ({ criterion: c.name, score: null, max: c.points, note: "" }));
              const got = scored ? cur.rubric.reduce((s, r) => s + (Number(r.score) || 0), 0) : null;
              return (
                <div style={{ padding:"14px 12px", borderBottom:`1px solid ${T.border}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:9, paddingLeft:4 }}>
                    <span style={{ fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em", color:T.muted }}>Rubric Score</span>
                    <span style={{ fontSize:12, fontWeight:700, color: scored ? T.navy : T.muted }}>
                      {scored ? `${got}/${rubricMax}` : `–/${rubricMax}`}
                    </span>
                  </div>
                  {rows.map((r, i) => (
                    <div key={i} style={{ padding:"7px 0", borderTop: i ? `1px solid ${T.border}` : "none" }}>
                      <div style={{ display:"flex", justifyContent:"space-between", gap:8 }}>
                        <span style={{ fontSize:12, color:T.ink, fontWeight:600, lineHeight:1.4 }}>{r.criterion}</span>
                        <span style={{ fontSize:12, fontWeight:700, flexShrink:0, color: r.score==null ? T.muted : T.gold }}>
                          {r.score==null ? `–/${r.max}` : `${r.score}/${r.max}`}
                        </span>
                      </div>
                      {r.note && <div style={{ fontSize:11, color:T.inkLight, marginTop:3, lineHeight:1.5 }}>{r.note}</div>}
                    </div>
                  ))}
                  {!scored && (
                    <div style={{ fontSize:11, color:T.muted, fontStyle:"italic", marginTop:6 }}>
                      Scored against the rubric once this question is answered.
                    </div>
                  )}
                </div>
              );
